...
package ly.seb.arwormhole
{
	...
	public class Speaker extends Cylinder
	{
		...
		public var depthVel : Number = 0; 
		public var targetDepth : Number = 0; 
		public var counter : int = 0; 
		
		public var hsegs : int = 4; 
		public var vsegs : int = 4; 
		
		
		public function Speaker(bitmap : BitmapData, scalex : Number, scaley : Number, radius:Number=100)
		{
			this.bitmap = bitmap; 
...
//			mat.doubleSided = true; 
			super(mat, radius, 0.01, hsegs, vsegs, 0, false, false);
			
			rotationZ = 45; 
			
			referenceDisc = new Cylinder(new ColorMaterial(0xff0000,0.1), radius, 0.01, hsegs, vsegs,0,false,false);
		...
		public function twist(deg : Number, targetdepth : Number) : void
		{
			...
			depthVel*=0.9; 
			depthVel+= ((targetDepth-depth)*0.6); 
			depth+=depthVel;
			
			counter ++; 
			var tempdepth : Number = depth * (((counter%2)*0.5)+0.5);
			...
			for(var i:int = 0; i<vertexcount ; i++)
			{
				...
				pos.y =  easeOut(distance, 0, tempdepth, 4); 
		...
		public static function easeInOut (t:Number, b:Number, c:Number, pow:int = 2, d:Number = 1):Number {
			if ((t/=d/2) < 1) return c/2*t*t + b;
			return -c/2 * ((--t)*(t-2) - 1) + b;
		}
		public static function easeOut (t:Number, b:Number, c:Number, pow:int = 2, d:Number = 1):Number {
			return -c *(t/=d)*(t-2) + b;
		}
